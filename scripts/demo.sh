#!/bin/sh
# Fires a comprehensive sequence of notifications to exercise every
# feature of the daemon. Requires notchify-daemon to be running.
#
...
#
# Sections can be selected via the first positional argument
# (singletons, persistent, stacks, overflow, hover, queue, focus,
# bodies, edge, all). Default is "all".
#
# This is BOTH the demo recording script and the scripted regression
# test. Walk through it before merging; the cases that require human
# eyes (animation smoothness, accessibility tooling, multi-display)
# are listed in the manual checklist at the end of this file.
set -eu

# Prefer the in-tree debug build over a system-installed `notchify`
# so `./scripts/demo.sh` exercises the current branch's CLI without
...
# --- Singletons (basic a→b→c→d→e→f lifecycle) --------------------

if run_section singletons; then
    echo "[singletons] basic lifecycle, sounds, colors, icons"
    ...
    # No body — tests the empty-body path (uses extraHeight, not two-line).
    "$N" "Title only"
    sleep 6
    ...
    # Plain ungrouped (no icon/color) — coalesces into a:_default chip.
    "$N" "Plain one"
    sleep 1
